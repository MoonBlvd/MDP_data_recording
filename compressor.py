...
if __name__ == '__main__':
    ...
    file_path = '../Smart_Black_Box/data/Data_10172017/videos/*'
    all_files = glob.glob(file_path)

    ext = sys.argv[1]
    F = open('compression_ratio_quality_WV.csv', 'w')
    writer = csv.writer(F)
    ...
    for video_file in all_files:
        ...
        frame_ctr = 0
        frames_to_watch = np.random.permutation(1700)
        #while cap.isOpened():
        all_output = []

        for img_id in frames_to_watch[:2]:
